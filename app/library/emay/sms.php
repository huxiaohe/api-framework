...
class b2c_emay_sms{
	 ...
	 * 序列号,请通过亿美销售人员获取
	 */
	var $serialNumber = '6SDK-EMY-6688-KIWUL';
	...
	*/	
	...
	function send($to,$title,$message,$config = ""){
		$message = '【奥德意】'.$message;
		$to = array($to);
		$statusCode = $this->client->sendSMS($to,$message);
		return $statusCode;
	}
	...
	function login()
	{
		 ...
		 * 我们建议 sesson key不用常变
		 */
		// $this->sessionKey = $this->client->generateKey();
	...
	function sendSMS()
	{
		 ...
		 */
		$statusCode = $this->client->sendSMS(array('13641882176'),"test2测试");
		echo "处理状态码:".$statusCode;
	}
